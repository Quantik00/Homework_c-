...
// Шаблон для сложения трёх элементов одного типа, T& -- ссылка, template - шаблон
template <typename T> T summ(T& first_elem, T& second_elem, T& third_elem) {
...
public:
    ...
    // Метод для улучшения книги добавлением комбинированного 
...
public:
    ...
    void use(MagicBook* book, int num, Beastes* target) {
        if (book->m_spells[num]->level <= level) {  // Проверка, если уровень заклинания подходит 
            book->m_spells[num]->ability(target);  // Применяем заклинание к цели, вот это стратегия
        } else {
...
int main() {
    ...
    try {    // Попытка ввода 
        for (int i = 0; i < number_of_spells; i++) {
            ...
            } // Вот это всё был фабричный метод 
            else {
                throw(2);  // Исключение: нет такого, шаблон исключение 
            }
        }
    }
    catch (int x) {  
